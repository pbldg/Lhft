...
**************************/
        var strHtml1="<br><font color='#FF00FF'>打印控件未安装!点击这里<a href='download/install_lodop.exe'>执行安装</a>,安装后请刷新页面或重新进入。</font>";
        var strHtml2="<br><font color='#FF00FF'>打印控件需要升级!点击这里<a href='download/install_lodop.exe'>执行升级</a>,升级后请重新进入。</font>";
...
function CheckIsInstall() {	
	try{
		...
		if ((LODOP!=null)&&(typeof(LODOP.VERSION)!="undefined")) alert("本机已成功安装过打印控件!");
	}catch(err){
		alert("Error:本机未安装打印控件，请下载安装!");
 	}
}

// 打印预览
function prn_preview(areaId) {	
	CreatePrintPage(areaId);	
	LODOP.PREVIEW();	
};
// 直接打印
function prn_print(areaId) {		
	CreatePrintPage(areaId);
	LODOP.PRINT();	
};
// 选择打印机
function prn_printA(areaId) {		
	CreatePrintPage(areaId);
	LODOP.PRINTA(); 	
};	
