 ...
 *
 * License
 * http://www.e9rj.com/license
 *
 * Date: 30 Aug, 2011 16:39:39 +0300
 */
(function(f) {
    function e(h) {
    ...
    if (/1\.(0|1|2|3|4)\.(0|1)/.test(f.fn.jquery) || /^1.1/.test(f.fn.jquery) || /^1.2/.test(f.fn.jquery) || /^1.3/.test(f.fn.jquery)) alert("tn3gallery requires jQuery v1.4.2 or later!  You are using v" + f.fn.jquery);
        ...
        k = [];
        f.fn.tn3 = function(h) {
            f.each(["skin", "startWithAlbums", "external"],
            function(l, m) {
                var p = m.split(".");
    ...
    f.fn.tn3.Gallery.prototype = {
        ...
        parseLayout: function() {
            ...
            $cm = this.$c;
            f.each(["albums", "album", "album-next", "album-prev", "show-albums", "timer"],
            function(h, j) {
                delete a[j];
                $cm.find("." + c.cssID + "-" + j).remove()
            });
            var k = f('<div title="Powered by e9rj"></div>');
        ...
        setAlbumData: function(a, c) {
            if (c) this.trigger("error", {
                ...
                for (var d = 0,
                b = a.length; d < b; d++) this.data.push(a[d]);
        ...
        fullscreen: function() {
            if (this.config.isFullScreen) {
                ...
                if(this.config.width !== null || this.config.height !== null){
                	...
                	this.resize(this.config.initValues.width, this.config.initValues.height);
                }
                
                if (this.items.fullscreen) {
                    this.items.fullscreen.removeClass(this.config.cssID + "-fullscreen-active");
                    this.items.fullscreen.attr("title", this.t("最大化"))
                }
            ...
            try{fullscreencallback();}catch(e){}
        },
        onEscape: function(a) {
        ...
        resize: function(a, c) {
            ...
            f.each(this.special.rh,
            function(k, h) {
                n.items[h].width(n.config.initValues[h].w + d)
            });
            f.each(this.special.rv,
            function(k, h) {
        ...
        center: function() {
            var a, c = this;
            f.each(this.special.v,
            function(d, b) {
                a = c.items[b];
                a.css("top", (a.parent().height() - a.height()) / 2)
            });
            f.each(this.special.h,
            function(d, b) {
                a = c.items[b];
        ...
        replaceMenu: function(a, c) {
            var d = '<div style="position:absolute;background-color:#fff;color: #000;padding:0px 4px 0px 4px;z-index:1010;font-family:sans-serif;font-size:12px;">&copy; <a href="' + c + '">' + a + "</a></div>";
            this.$tn3.bind("contextmenu",
            function(b) {
                b.preventDefault()
            }).bind("mousedown",
            function(b) {
    ...
    f.fn.tn3.Thumbnailer.prototype = {
        ...
        loadNextThumb: function() {
            ...
            if (this.config.mode == "thumbs") {
                ...
                if (a) {
                    this.loaders.push(new f.fn.tn3.ImageLoader(a, this, this.onLoadThumb, [e, this.thumbCount])); ! this.config.seqLoad && this.thumbCount < this.data.length - 1 && this.loadNextThumb();
                    return
        ...
        rebuild: function(e) {
            ...
            this.loaders !== null && f.each(this.loaders,
            function(a, c) {
                c.cancel()
            });
            this.init()
...
})(jQuery); (function(f) { (f.fn.tn3.External = function(e, a) {
        if (e) {
            this.context = a;
            this.reqs = e.length;
