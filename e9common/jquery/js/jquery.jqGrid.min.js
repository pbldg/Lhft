				...
				if(ts.p.cellEdit === true) {
					...
					if(ts.p.multiselect && ts.p.multiboxonly) {
						if(scb){$(ts).jqGrid("setSelection",ri,true,e);}
							...
							//begin update by zhoudc, 20140-08-23, 原因：需要选中多行时， 点击复选框，如果不小心点到该单元格之内复选框之外的的区域时，很容易把之前选中的项都清空
							if(ci == 0 && $(":checkbox", td).length > 0){
								$(":checkbox", td).attr("checked");
								$(ts).jqGrid("setSelection",ri,true,e);
								return;
							}
							//end
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			if(shrink===true) {
				...
				var ths = $htable.find(".jqg-first-row-header th");
				...
				$.each($t.p.colModel, function(i) {
					if(this.hidden === false && !this.fixed){
						...
						//设置标题列宽度，主要用于解决因setGroupHeaders而引起的数据列边框与标题列边框不对齐
						if(ths && ths[i]) {							
							$(ths[i]).css("width", cw + "px");							
						} 	
				...
				//设置最后一列标题列宽度，主要用于解决因setGroupHeaders而引起的数据列边框与标题列边框不对齐
				if(ths && ths[lvc]) {			
					$(ths[lvc]).css("width", cw + "px");
				}
